...
class ContratadoSelect2widgetForm(forms.ModelForm):
	class Meta:
		model = Contratado
		exclude = []
		field = ('cliente', 'estatus')
		widgets = {
			'cliente': Select2Widget,
			'estatus': CheckboxInput,
...
class ContratadoAdmin(admin.ModelAdmin):
	...
	list_display_links = ['numcontrato']
	list_display = ['numcontrato', 'estatus',  'cliente', 'cedula', 'telefono_o' , 'fecha_contratacion' , 'fecha_expiracion', 'servicio']

	fieldsets= [
		(None, {'fields': [()]}),
		('CLIENTE A CONTRATAR', {'fields': [('cliente', 'estatus')]}),
		('DATOS PERSONALES DEL CLIENTE', {'fields': ('cedula', 'fecha_n', 'edad', 'telefono_o', 'rif')}),
		('DATOS DE BUSQUEDA', {'fields': [('estado', 'municipio', 'parroquia'), ('sector','nombre_sector'), ('ubicacion', 'nombre_ubicacion'),('vivienda','nombre_vivienda'),('piso','numero'), ('punto_referencia'), ]}),
		(None, {'fields': [()]}),
		('DATOS DE SALUD', {'fields': [('cancer', 'diabetes'), ('enfermedad_corazon', 'presion_arterial'),('enfermedad_renal','enfermendad_mental'),'enfermedades_importantes',('salud','peso','estatura'),('enfermedad_respiratoria','enfermedad_digestivo'),('enfermedad_circulatorio','otras_enfermedades')]}),
		('SERVICIO A CONTRATAR', {'fields': ['servicio','precio_total']}),
	]
